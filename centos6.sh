...
echo "<pre>Setup by Office48 | IndoPhreak.com | @NyxGodOfNight </pre>" > /home/vps/public_html/index.html
...
cd /etc/openvpn/
tar xf openvpn.tar
wget -O /etc/openvpn/1194.conf "https://raw.githubusercontent.com/Office48/VPSScript/master/conf/1194-centos.conf"
if [ "$OS" == "x86_64" ]; then
  wget -O /etc/openvpn/1194.conf "https://raw.githubusercontent.com/Office48/VPSScript/master/conf/1194-centos64.conf"
fi
...
useradd -M -s /bin/false Office
echo "UPILServer:$PASS" | chpasswd
echo "UPILServer" > pass.txt
echo "$PASS" >> pass.txt
...
curl "https://raw.githubusercontent.com/Office48/VPSScript/master/conf/mrtg.conf" >> /etc/mrtg/mrtg.cfg
...
# install squid
yum -y install squid
wget -O /etc/squid/squid.conf "https://raw.githubusercontent.com/Office48/VPSScript/master/conf/squid-centos.conf"
...
# pasang bmon
if [ "$OS" == "x86_64" ]; then
  wget -O /usr/bin/bmon "https://raw.githubusercontent.com/Office48/VPSScript/master/conf/bmon64"
else
  wget -O /usr/bin/bmon "https://raw.githubusercontent.com/Office48/VPSScript/master/conf/bmon"
fi
...
curl https://raw.githubusercontent.com/Office48/VPSScript/master/conf/user-login.sh > user-login.sh
curl https://raw.githubusercontent.com/Office48/VPSScript/master/conf/user-expire.sh > user-expire.sh
...
# info
clear
echo "IndoPhreak.com | @NyxGodOfNight | Office48 " | tee log-install.txt
...
echo "---------------"  | tee -a log-install.txt
echo "User     : UPILServer"  | tee -a log-install.txt
